...
#if defined(CONFIG_MACH_AMAZING) || defined(CONFIG_MACH_KYLE) || defined(CONFIG_MACH_ROY) ||  \
	defined(CONFIG_MACH_AMAZING_CDMA) || defined(CONFIG_MACH_ARUBA_OPEN) || defined(CONFIG_MACH_ARUBASLIM_OPEN) || defined(CONFIG_MACH_ARUBA_CTC) || defined(CONFIG_MACH_KYLEPLUS_OPEN)|| defined(CONFIG_MACH_KYLEPLUS_CTC)
...
#define FSA880_REG_DEV_T1_MASK	(DEV_DEDICATED_CHG | DEV_CAR_KIT | DEV_USB)
...
#if defined(CONFIG_MACH_INFINITE_DUOS_CTC) || defined(CONFIG_MACH_ARUBA_DUOS_CTC) || defined(CONFIG_MACH_DELOS_DUOS_CTC) || defined(CONFIG_MACH_BAFFIN_DUOS_CTC)
...
static void fsa9485_uart_sel_switch_en(bool en)
{
	...
	{	
		...
		mdelay(10);
		printk("%s read GPIO %d UART_SEL : value = %d \n", __func__,GPIO_UART_SEL,gpio_get_value(GPIO_UART_SEL));	
	}
}
#endif
#endif
				...
				char *buf)
{
	...
	if ((fd = sys_open("/persist/uart_sel.bin", O_RDONLY,0)) < 0) {
		printk("[FSA9480]: %s :: open failed %s ,fd=0x%x\n", __func__, "/persist/uart_sel.bin", fd);
		return 0;
	}
	...
	switch(curr_uart_path)
	{
		...
		default:
			sprintf(buf, "%s\n", "PDA");
			printk("[FSA9480]: UART default MASTER\n");
			break;
	}
...
static ssize_t uart_sel_store(struct device *dev, struct device_attribute *attr,const char *buf, size_t size)
{
	...
	if ((fd = sys_open("/persist/uart_sel.bin", O_CREAT|O_WRONLY, 0)) < 0)
	{
...
#if defined(CONFIG_MACH_KYLE_I) ||defined(CONFIG_MACH_ARUBA_OPEN) || defined(CONFIG_MACH_KYLE_CHN)|| defined(CONFIG_MACH_ARUBA_CTC)|| defined(CONFIG_MACH_KYLEPLUS_OPEN)|| defined(CONFIG_MACH_KYLEPLUS_CTC) /* since certain dock couldn't be recognized as a charger */
u8 intr2_av  = 0;
#endif
...
/* WORKAROUND for not recognizing ta detaching */
	...
	if  (!intr) {
		...
		if (val1 == 0 || val2 == 0) {
			...
			/* reset USB switch */
			fsa9480_write_reg(client, FSA_REG_CTRL, 0x1E);
		}
}
#endif
...
#if  defined(CONFIG_TOUCHSCREEN_MELFAS_KYLE)  
// defined(CONFIG_MACH_ARUBA_DUOS_CTC) || defined(CONFIG_MACH_ARUBA_OPEN) || defined(CONFIG_MACH_ARUBASLIM_OPEN) || defined(CONFIG_MACH_KYLEPLUS_CTC) || defined(CONFIG_MACH_KYLEPLUS_OPEN) || defined(CONFIG_MACH_INFINITE_DUOS_CTC) || defined(CONFIG_MACH_DELOS_DUOS_CTC)
		if((usbsw->dev1 & DEV_DEDICATED_CHG) && (!(val1 & DEV_DEDICATED_CHG))){
			touch_ta_noti(0);
		}
#endif		
	} else if (intr == 0) {
...
static void sec_switch_init_work(struct work_struct *work)
{
	...
	if ((fd_usb = sys_open("/persist/usb_sel.bin", O_RDWR  ,0664)) < 0)	{
		fd_usb = sys_open("/persist/usb_sel.bin", O_CREAT|O_RDWR  ,0664);
		sys_write(fd_usb, buffer, 1);
	}
		
	if ((fd_uart = sys_open("/persist/uart_sel.bin", O_RDWR  ,0664)) < 0)	{
		fd_uart = sys_open("/persist/uart_sel.bin", O_CREAT|O_RDWR	,0664);
		sys_write(fd_uart, buffer, 1);
	}

	if (fd_usb < 0 || fd_uart < 0)
	{ 
		schedule_delayed_work(&wq->work_q, msecs_to_jiffies(1000));
		printk("[FSA9480]: %s :: open failed %s ,fd=0x%x\n",__func__,"/persist/usb_sel.bin",fd_usb);
		...
		printk("[FSA9480]: %s :: open success %s ,fd=0x%x\n",__func__,"/persist/usb_sel.bin",fd_usb);
		printk("[FSA9480]: %s :: open success %s ,fd=0x%x\n",__func__,"/persist/uart_sel.bin",fd_uart);		
	}	
	...
	if (!strcmp(buffer, "0"))
	{
		...
		fsa9480_set_switch("AUTO");
		curr_usb_path = SWITCH_MODEM;	
#if defined(CONFIG_MACH_ARUBA_DUOS_CTC)	|| defined(CONFIG_MACH_KYLEPLUS_CTC) || defined(CONFIG_MACH_BAFFIN_DUOS_CTC)
...
static int fsa9480_resume(struct i2c_client *client)
{
	...
	{
		intr = 1 << 0;
#if defined(CONFIG_MACH_KYLE_I) || defined(CONFIG_MACH_ARUBA_OPEN) || defined(CONFIG_MACH_KYLE_CHN)|| defined(CONFIG_MACH_ARUBA_CTC)|| defined(CONFIG_MACH_KYLEPLUS_OPEN)|| defined(CONFIG_MACH_KYLEPLUS_CTC) || defined(CONFIG_MACH_DELOS_DUOS_CTC)/* since certain dock couldn't be recognized as a charger */	
		if(val2)
			intr2_av = 1 << 0;
#endif		
	}	
