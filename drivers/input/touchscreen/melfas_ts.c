...
#elif defined(CONFIG_MACH_DELOS_DUOS_CTC) || defined(CONFIG_MACH_DELOS_OPEN)
#define MELFAS_MMS136_4P66
#elif defined(CONFIG_MACH_KYLEPLUS_CTC) || defined(CONFIG_MACH_KYLEPLUS_OPEN) || defined(CONFIG_MACH_INFINITE_DUOS_CTC) || defined(CONFIG_MACH_DELOS_DUOS_CTC)
...
{
	printk(KERN_ERR "%s %s\n", __func__, (en) ? "on" : "off");
#if (defined(CONFIG_MACH_ARUBA_OPEN) && !defined(CONFIG_MACH_KYLEPLUS_OPEN)) || defined(CONFIG_MACH_ARUBASLIM_OPEN) ||  defined(CONFIG_MACH_NEVIS3G)
				...
				if (touchType == TOUCH_SCREEN) {
					...
					if ((buf[i] & 0x80) == 0)
						g_Mtouch_info[FingerID].strength = 0;
							...
							if(g_Mtouch_info[FingerID].posX > TS_MAX_X_COORD || g_Mtouch_info[FingerID].posX < 1 || g_Mtouch_info[FingerID].posY > TS_MAX_Y_COORD || g_Mtouch_info[FingerID].posY < 1)
								g_Mtouch_info[FingerID].strength = -1;		// data is ignored, just down event.
					else
...
#if defined(CONFIG_MACH_ARUBA_DUOS_CTC) || defined(CONFIG_MACH_ARUBA_OPEN) || defined(CONFIG_MACH_NEVIS3G)
	threshold = 20;
#endif
	return snprintf(buf, sizeof(buf), "%d\n", threshold);
}
...
#if defined(CONFIG_MACH_ARUBA_DUOS_CTC) || (defined(CONFIG_MACH_ARUBA_OPEN) && !defined(CONFIG_MACH_KYLEPLUS_OPEN)) || defined(CONFIG_MACH_DELOS_OPEN) || defined(CONFIG_MACH_ARUBASLIM_OPEN)
	unsigned char data;
	int int_data;
	int errnum;	
...
static int check_delta_value(struct melfas_ts_data *ts)
{
	...
	setLowLevelData[1] = 0x40; /* UNIVCMD_ENTER_TEST_MODE */
	ret = melfas_i2c_write(ts->client, setLowLevelData, 2);
#if defined(CONFIG_MACH_KYLEPLUS_CTC) || defined(CONFIG_MACH_KYLEPLUS_OPEN) || defined(CONFIG_MACH_INFINITE_DUOS_CTC) || defined(CONFIG_MACH_DELOS_DUOS_CTC) || defined(CONFIG_MACH_NEVIS3G)	// erase by melfas klski98
	while (gpio_get_value(GPIO_TOUCH_INT))
		;
...
static int melfas_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts->config_fw_version = "S6252_Me_1016";
