...
static int _fan54013_set_iterm(void)
{
	u8 value;
	...
	if (charger_ic->VRsense == FAN54013_VRSENSE_68m) {	
		value = value | ((charger_ic->Iterm) << FAN54013_SHIFT_ITERM);
		pr_info("%s: ITERM %smA\n", __func__, fan54013_68m_iterm[charger_ic->Iterm]);
	} else {
		value = value | ((charger_ic->Iterm) << FAN54013_SHIFT_ITERM);
		pr_info("%s: ITERM %smA\n", __func__, fan54013_100m_iterm[charger_ic->Iterm]);
	}
...
#ifdef DISABLE_T32SEC_TIMER
#if !defined(CONFIG_MACH_BAFFIN_DUOS_CTC) /* move to bootloader */
