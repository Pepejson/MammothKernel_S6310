...
static int _bq24157_charging_control(int enable)
{
	...
	if (enable) {		
		...
		pmic_gpio_set_value(PMIC_GPIO_10, 0);	/* Set CD pin LOW */
#endif
		value = value |
...
#if defined(CONFIG_MACH_DELOS_OPEN)
		pmic_gpio_set_value(PMIC_GPIO_10, 1);	/* Set CD pin HIGH */
#endif
		value = value |
