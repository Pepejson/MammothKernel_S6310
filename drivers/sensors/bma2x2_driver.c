		...
		const char *buf, size_t count)
{
	...
	pr_info("%s: %d \n reactive_enable = %d, reactive_enable =%d\n", __func__, data, atomic_read(&bma2x2->reactive_enable), atomic_read(&bma2x2->reactive_state));
#endif
...
#if !defined(CONFIG_MACH_ARUBA_OPEN) && !defined(CONFIG_MACH_ARUBASLIM_OPEN) && !defined(CONFIG_MACH_ARUBA_DUOS_CTC) && !defined(CONFIG_MACH_BAFFIN_DUOS_CTC) && !defined(CONFIG_MACH_INFINITE_DUOS_CTC) && !defined(CONFIG_MACH_DELOS_DUOS_CTC)
