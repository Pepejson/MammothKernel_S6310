...
void bma222e_activate(int flgatm, int flg, int dtime)
{
...
#if 0//def CONFIG_USE_BMA_INITLIB
	...
	if (flg != 0) {
		...
		msleep(20);
		/*
		Whenever a soft reset of bma250e is performed by 
		writing SOFT_RESEET(0xB6) to the register BMA222E_REG14(0x14),
		the backup algorithm must be executed,this is for the POR issue.
		*/	
#ifdef CONFIG_USE_BMA_INITLIB
		if (probe_done == PROBE_SUCCESS) {
			pr_info("[acc] %s\n", __func__);
