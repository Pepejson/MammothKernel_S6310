...
static int mipi_hx8369b_lcd_on(struct platform_device *pdev)
{
	...
	if (mipi->mode == DSI_VIDEO_MODE) {
		if(read_id==HX8369B_PANEL1) {
			...
			if(ret==0)
				goto MIPI_ERROR;
		} else if(read_id==HX8369B_PANEL_BOE1 ||read_id==HX8369B_PANEL_BOE2) {
					...
					u32 channel, u32 panel)
{
	...
	if(read_id==HX8369B_PANEL_BOE1 || read_id==HX8369B_PANEL_BOE2) {
