...
#define HX8369B_SLEEP_IN_DELAY 	150
#define HX8369B_SLEEP_OUT_DELAY	150
...
//	hx8369b_lcd_set_temperature();
	...
	if (mipi->mode == DSI_VIDEO_MODE) {
		...
		if (rotate) {
			ret  = mipi_dsi_cmds_tx(&hx8369b_tx_buf, hx8369b_video_display_on_cmds_rotate, ARRAY_SIZE(hx8369b_video_display_on_cmds_rotate));
                        if(ret==0)
			        goto MIPI_ERROR;
                 }
	}
  ...
  DPRINT("%s [mDNIe] negative=%d\n", __func__, mDNIe_cfg.negative);
	...
	if(tuning_enable && hx8369b_video_display_mDNIe_tune_cmds[0].dlen > 0) {
        ...
        // Display on
	msleep(8);
	ret = mipi_dsi_cmds_tx(&hx8369b_tx_buf, BAFFIN_hx8369b_video_display_init_rev1_3_cmds, ARRAY_SIZE(BAFFIN_hx8369b_video_display_init_rev1_3_cmds));
        if(ret==0)
		goto MIPI_ERROR;
	msleep(200);
	ret = mipi_dsi_cmds_tx(&hx8369b_tx_buf, hx8369b_video_display_on_smd_cmds, ARRAY_SIZE(hx8369b_video_display_on_smd_cmds));
        if(ret==0)
		goto MIPI_ERROR;
