...
#if defined(CONFIG_MACH_ARUBA_OPEN) || defined(CONFIG_MACH_ARUBA_CTC) || defined(CONFIG_MACH_KYLEPLUS_CTC) || defined(CONFIG_MACH_ARUBA_DUOS_CTC)
...
#ifdef CONFIG_MSM_PMIC_VIBRATOR
	int rc = 0;
#endif /* CONFIG_MSM_PMIC_VIBRATOR */
...
&& !defined(CONFIG_GPIO_CONTROL_VIBRATOR)	
	...
	if (rc) {
		pr_err("[VIB] %s: set mode failed", __func__);
		return;
	}

	rc = pmic_vib_mot_set_volt(vib_level);
	if (rc) {
		pr_err("[VIB] %s: set voltage failed", __func__);
	}

#else

#if (defined(CONFIG_MACH_ARUBA_OPEN) || defined(CONFIG_MACH_ROY))   \
&& !defined(CONFIG_GPIO_CONTROL_VIBRATOR)	
	if (on) {
