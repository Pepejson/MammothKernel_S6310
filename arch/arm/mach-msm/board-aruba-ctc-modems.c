...
void config_gsm_modem_gpio(void)
{
	...
	if (gpio_phone_active) {
		...
		if (err) {
			...
			gpio_tlmm_config(GPIO_CFG(gpio_phone_active, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
...
#if defined(CONFIG_MACH_ARUBA_DUOS_CTC) || defined(CONFIG_MACH_INFINITE_DUOS_CTC)
